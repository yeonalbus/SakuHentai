...
// 1. 生成 72 条本地历史记录（刚好 3 页）
const historyComics = ref<OfflineComic[]>(
  generateOfflineComics(72).map((comic, i) => ({
    ...comic,
    id: `offline-his-${i + 1}`,
    title: `📖 [本地历史] ${comic.title.replace(/^📖\s*\[本地扫描\]\s*/, '')}`,
    updatedAt: `2026-07-29 ${22 - Math.floor(i / 3)}:15`,
  })),
)
...
const handleClearHistory = async () => {
  const confirmed = await modal.confirm('确定要清空所有的离线阅读历史记录吗？', '清空历史')
  if (confirmed) {
    historyComics.value = []
    toast.info('本地阅读历史记录已清空')
  }
}
</script>
